...
namespace MVC_DynamicMenu.Repo
{
    public class Patient_ContactRepo
    {
		...
		*/
		...
		public Patient GetAllpatientInfoByCity(String city)
		{
			List<Patient> obj = _c.Patient.FromSqlRaw("Select * from dbo.Patient where Participant_City=" + city).ToList();
			obj[0].Patient_Info = _c.Patient_Info.FromSqlRaw("Select * from dbo.Patient_Info where Participant_City=" + city).ToList();
			obj[0].patient_Contacts = _c.Patient_Contact.FromSqlRaw("Select * from dbo.patient_Contact where Participant_City=" + city).ToList();
			obj[0].summerisings = _c.Summerising.FromSqlRaw("Select * from dbo.summerising where Participant_City=" + city).ToList();
			obj[0].patient_Helths = _c.Patient_Helth.FromSqlRaw("Select * from dbo.patient_Helth where Participant_City=" + city).ToList();
			obj[0].programs = _c.Programs.FromSqlRaw("Select * from dbo.programs where Participant_City=" + city).ToList();
			obj[0].Case_Worker = _c.Case_Worker.FromSqlRaw("Select * from dbo.Case_Worker where Participant_City=" + city).ToList();
			obj[0].Housing_Register_Application = _c.Housing_Register_Application.FromSqlRaw("Select * from dbo.Housing_Register_Application where Participant_City=" + city).ToList();
			obj[0].CareTeams = _c.CareTeams.FromSqlRaw("Select * from dbo.CareTeams where Participant_City=" + city).ToList();
			//obj[0].Files = _c.File.FromSqlRaw("Select * from dbo.File where PatientID=" + id).ToList();
			obj[0].Commencement_Issuse = _c.Commencement_Issuse.FromSqlRaw("Select * from dbo.Commencement_Issuse where Participant_City=" + city).ToList();
			return obj[0];
		}

		public List<Patient> GetpatientInfoByCity()
		{
			List<Patient> obj = _c.Patient.FromSqlRaw("Select * from dbo.Patient where Participant_City=" + "'Melbourne'").ToList();
			return obj;
		}
	}
}
